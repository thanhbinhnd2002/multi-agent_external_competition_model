...
# Đề xuất áp dụng: Giữ trạng thái alpha cố định như Beta để đảm bảo alpha không bị ảnh hưởng trong quá trình cập nhật
...
def simulate_competition(G, attach_nodes, x_prev=None, alpha_idx=None):
    ...
    if alpha_idx is not None:
        alpha_node_name = list(G.nodes())[alpha_idx]
        alpha_idx_in_new = node_order.index(alpha_node_name)
        fixed_nodes.add(alpha_idx_in_new)  # Giữ trạng thái alpha cố định
    ...
    for i, attach_node in enumerate(attach_nodes):
        ...
        x[beta_idx] = -1  # Đã sửa: gán trạng thái cho node Beta
